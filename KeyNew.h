...
public:
  ...
  void sendKeyStroke(byte keyStroke, byte modifiers) {
    uint8_t keyNon[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    uint8_t sendKe[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    sendKe[0] = modifiers;
    sendKe[2] = keyStroke;
    Serial.write(sendKe, 8);          // Keycode 6  

    Serial.write(keyNon, 8);    // Release Key   
  }
  ...
  void sendKeyReleased(byte key1, byte modifiers) {
    for (int i=1; i<8; i++) {
    if (sendKey[i] == key1) {
      sendKey[i] = 0;
    }
